...
module.exports = class Autor {
	...
	verificarDatos( datos ) {
		return new Promise( ( resuelve, rechaza ) => {
			...
			// Verifica
			if ( datos.uid == 0 ) {
				rechaza( new modError.ErrorEstado( this.msj.errorUid, 400 ) ); return;
			}
			if ( datos.nombreAutor == '' ) {
				rechaza( new modError.ErrorEstado( this.msj.faltaFirmaAutor, 400 ) ); return;
			}
			if ( datos.autorBase == '' ) {
				rechaza( new modError.ErrorEstado( this.msj.errorFirmaAutor, 400 ) ); return;
			}
			if ( datos.nombreUsuario == '' ) {
				rechaza( new modError.ErrorEstado( this.msj.faltaNombreUsr, 400 ) ); return;
			}
			resuelve( datos );
		});
	}
