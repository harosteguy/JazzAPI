    ...
    return new Promise((resolve, reject) => {
      ...
      if (Array.isArray(datos.idsCat)) {
        ...
        if (datos.idsCat.length > 0) { // Si queda algún id en el array
          ...
          let consulta = 'select id from blog_categorias where idBlog = ? and id in ' + listaIds
  ...
  verificarDatos (datos) {
    return new Promise((resolve, reject) => {
      this.sanear(datos).then(sanos => { // Primero sanea
        if (sanos.titulo === '') {
          reject(new modError.ErrorEstado(this.msj.faltaTitulo, 400)); return
        }
        if (sanos.fechaPublicado) {
          if (!utiles.esFechaHora(sanos.fechaPublicado)) { // La fecha es incorrecta
            reject(new modError.ErrorEstado(this.msj.errorFechaPublicado, 400)); return
          }
        } else { // No hay fecha
        ...
        if (sanos.auxFecha) {
          if (!utiles.esFechaHora(sanos.auxFecha)) {
            reject(new modError.ErrorEstado(this.msj.errorAuxFecha, 400)); return
          }
        }
        resolve(sanos)
      }).catch(error => { reject(error) })
  ...
  borrarImagenes (blogBase, idArti) {
    return new Promise((resolve, reject) => {
      ...
      fs.readdir(carpetaImagenes, (error, archivos) => {
        if (error) {
          if (error.code === 'ENOENT') resolve(true) // La carpeta no existe, nada que borrar
          else reject(error)
          return
        }
